...
namespace BusinessCentral.LinterCop.Design
{
    ...
    public class Rule28AnalyzeGetInvocations : DiagnosticAnalyzer
    {
        ...
        public void AnalyzeGetCall(SyntaxNodeAnalysisContext context)
        {
            ...
            {
                context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0071_1IncorrectArgumentCount, invocationExpression.GetLocation(), baseTable.Name, primaryKeyFields.Length));
                return;
            }
            ...
            for (int i = 0; i < arguments.Count; i++)
            {
                ...
                if (fieldType.IsTextType() && argumentType != null && argumentType.IsTextType())
                {
                    ...
                    if (argumentLength > fieldType.Length)
                        context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0071_3ArgumentLengthExceedsMaxLength, arguments[i].GetLocation(), tableName, fieldType.Length));
                }
                else if (fieldType != argumentType)
                {
                    context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule0071_2InvalidArgumentTypeInGetCall, arguments[i].GetLocation(), tableName));
                }
            }
        }
    }
